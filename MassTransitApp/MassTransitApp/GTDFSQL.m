...
-(NSArray *)allTimesinGivenStation:(Station *)station
{
    ...
    sqlite3* DBconnection=[Database getConnection];
    NSMutableArray *arrayList=[[NSMutableArray alloc]init];
    NSString* query1 = @"SELECT * FROM stop_times INNER JOIN trips ON ";
    NSString* query2 = @"stop_times.trip_id = trips.trip_id WHERE stop_id = '";
    NSString* query3 = @"' ORDER BY stop_times.arrival_time;";
    NSString* query = [NSString stringWithFormat:@"%@%@%@%@",query1,query2,station.stationID, query3];
    sqlite3_stmt *statement;
    const unsigned char* text;
    
    NSString* arrivalTime;
    NSString* nextLocation;
    ...
    if (sqlite3_prepare_v2(DBconnection, [query UTF8String],
        ...
        while (sqlite3_step(statement) == SQLITE_ROW) {
            text = sqlite3_column_text(statement, 1);
            if (text)
                arrivalTime = [NSString stringWithCString:
                               (const char*)text encoding:NSUTF8StringEncoding];
            else
                arrivalTime = nil;
            text = sqlite3_column_text(statement, 9);
            if (text)
                nextLocation = [NSString stringWithCString:
                                (const char*)text encoding:NSUTF8StringEncoding];
            else
                nextLocation = nil;
            Times *tm=[[Times alloc]initWithStaionID:station.stationID AndnextLocation:nextLocation AndarrivalTime:arrivalTime];
            
            [arrayList addObject:tm];
        }
        sqlite3_finalize(statement);
    }
    return arrayList;
